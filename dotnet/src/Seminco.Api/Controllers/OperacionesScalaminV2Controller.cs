...
[ApiController]
[Route("api/operaciones-v2/scalamin")]
public sealed class OperacionesScalaminV2Controller(IOperacionScalaminV2Service service) : ControllerBase
{
    [HttpGet("aprobacion")]
    ...
    [HttpPost]
    [AllowAnonymous]
    public async Task<ActionResult<OperacionScalaminV2ResponseDto>> Create([FromBody] JsonElement body, CancellationToken ct)
        => StatusCode(StatusCodes.Status201Created, await service.CreateAsync(body, ct));

    [HttpPut("{id:int}")]
    [AllowAnonymous]
    public async Task<ActionResult<OperacionScalaminV2ResponseDto>> Update(int id, [FromBody] JsonElement body, CancellationToken ct)
    {
