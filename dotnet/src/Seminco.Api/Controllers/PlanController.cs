...
[ApiController]
[Authorize]
public abstract class PlanController<TDto>(IPlanService<TDto> service) : ControllerBase
    where TDto : class
{
    [HttpGet]
    ...
    [HttpGet("anio/{anio:int}/mes/{mes}")]
    public async Task<ActionResult<List<TDto>>> GetByYearAndMonth(int anio, string mes, CancellationToken ct)
    {
        var result = await service.GetByYearAndMonthAsync(anio, mes, ct);
        return Ok(result);
    }
}
