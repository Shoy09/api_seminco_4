...
[ApiController]
[Route("api/operaciones-v2/empernador")]
public sealed class OperacionesEmpernadorV2Controller(IOperacionEmpernadorV2Service service) : ControllerBase
{
    [HttpGet("aprobacion")]
    ...
    [HttpPost]
    [AllowAnonymous]
    public async Task<ActionResult<OperacionEmpernadorV2ResponseDto>> Create([FromBody] JsonElement body, CancellationToken ct)
        => StatusCode(StatusCodes.Status201Created, await service.CreateAsync(body, ct));

    [HttpPut("{id:int}")]
    [AllowAnonymous]
    public async Task<ActionResult<OperacionEmpernadorV2ResponseDto>> Update(int id, [FromBody] JsonElement body, CancellationToken ct)
    {
