...
public interface IOperacionCarguioV2Service
{
    Task<List<OperacionDto>> GetAllAsync(string? estado, string? envio, CancellationToken ct);
    Task<OperacionCarguioV2ResponseDto> CreateAsync(JsonElement body, CancellationToken ct);
    Task<OperacionCarguioV2ResponseDto?> UpdateAsync(int id, JsonElement body, CancellationToken ct);
