...
public interface IOperacionScissorV2Service
{
    Task<List<OperacionDto>> GetAllAsync(string? estado, string? envio, CancellationToken ct);
    Task<OperacionScissorV2ResponseDto> CreateAsync(JsonElement body, CancellationToken ct);
    Task<OperacionScissorV2ResponseDto?> UpdateAsync(int id, JsonElement body, CancellationToken ct);
