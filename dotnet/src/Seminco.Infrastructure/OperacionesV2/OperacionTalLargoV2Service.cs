...
public sealed class OperacionTalLargoV2Service(SemincoDbContext db) : IOperacionTalLargoV2Service
{
    ...
    private async Task<List<OperacionTalLargoRegistro>> BuildRegistrosAsync(List<RegistroRequest>? request, CancellationToken ct)
    {
        ...
        return request.Select(item =>
        {
            ...
            return new OperacionTalLargoRegistro
            {
                ...
                CreatedAt = now,
                UpdatedAt = now,
                Detalle = esOperativo ? new OperacionTalLargoRegistroDetalle { Nivel = item.Operacion.Nivel, TipoLabor = item.Operacion.TipoLabor, Labor = item.Operacion.Labor, Ala = item.Operacion.Ala, TalProd = ParseDecimal(item.Operacion.TalProd), TalRimados = ParseDecimal(item.Operacion.TalRimados), TalAlivio = ParseDecimal(item.Operacion.TalAlivio), TalRepaso = ParseDecimal(item.Operacion.TalRepaso), LongBarras = ParseDecimal(item.Operacion.LongBarras), NumBarras = ParseDecimal(item.Operacion.NumBarras), TipoPerforacion = item.Operacion.TipoPerforacion, TipoPerforacionId = item.Operacion.TipoPerforacionId, Observaciones = item.Operacion.Observaciones } : null
            };
        }).ToList();
    }
    ...
    private static OperacionDto MapToDto(OperacionTalLargo entity)
    {
        ...
        var registros = entity.Registros.Count == 0 ? null : ToJsonElement(entity.Registros.OrderBy(x => x.Id).Select(x => new RegistroRequest(x.ExternalId, x.Numero, x.EstadoPrincipal, x.CodigoEstado, x.HoraInicio.ToString("HH:mm:ss", CultureInfo.InvariantCulture), x.HoraFinal.ToString("HH:mm:ss", CultureInfo.InvariantCulture), x.Detalle is null ? new RegistroOperacionDetalleRequest(null, null, null, null, null, null, null, null, null, null, null, null, null) : new RegistroOperacionDetalleRequest(x.Detalle.Nivel, x.Detalle.TipoLabor, x.Detalle.Labor, x.Detalle.Ala, x.Detalle.TalProd?.ToString(CultureInfo.InvariantCulture), x.Detalle.TalRimados?.ToString(CultureInfo.InvariantCulture), x.Detalle.TalAlivio?.ToString(CultureInfo.InvariantCulture), x.Detalle.TalRepaso?.ToString(CultureInfo.InvariantCulture), x.Detalle.LongBarras?.ToString(CultureInfo.InvariantCulture), x.Detalle.NumBarras?.ToString(CultureInfo.InvariantCulture), x.Detalle.TipoPerforacion, x.Detalle.TipoPerforacionId, x.Detalle.Observaciones))).ToList());
